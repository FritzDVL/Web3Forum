...
import { PostReactionType, postId as toPostId, useSessionClient } from "@lens-protocol/react";
...
export function useForumVote({ publicationId, initialUpvotes, initialDownvotes }: UseForumVoteArgs) {
  ...
  useEffect(() => {
    async function checkVote() {
      ...
      const res = await fetch(`/api/forum/votes?publicationId=${publicationId}&account=${user.value.account}`);
      if (res.ok) {
  ...
  const vote = useCallback(
    async (direction: 1 | -1) => {
      ...
      try {
        ...
        const reactionType = direction === 1 ? PostReactionType.Upvote : PostReactionType.Downvote;
        ...
        if (userVote === direction) {
          ...
          if (userVote !== null) {
            const prevType = userVote === 1 ? PostReactionType.Upvote : PostReactionType.Downvote;
            await undoReaction(sessionClient.data, {
              post: pid,
              reaction: prevType,
            });
...
export async function POST(req: NextRequest) {
  ...
  if (direction === 0) {
    // Remove vote
    await db.from("forum_votes").delete().eq("publication_id", publicationId).eq("account_address", account);
  } else {
  ...
  useEffect(() => {
    async function check() {
      ...
      for (const groupAddr of [COMMONS_GROUP_ADDRESS, RESEARCH_GROUP_ADDRESS]) {
        ...
        if (result.isOk()) {
          const isAdmin = result.value.items.some(a => a.account.address === user.value.account);
          if (isAdmin) {
            setIsModerator(true);
            setIsLoading(false);
            return;
...
export async function POST(req: NextRequest, { params }: { params: { publicationId: string } }) {
  const db = await createClient();

  await db.from("forum_thread_replies").update({ is_hidden: true }).eq("publication_id", params.publicationId);
...
import { type Notification, evmAddress, useSessionClient } from "@lens-protocol/react";
  ...
  useEffect(() => {
    async function load() {
      ...
      const result = await fetchNotifications(sessionClient.data, {
        filter: {
          feeds: [{ feed: evmAddress(COMMONS_FEED_ADDRESS) }, { feed: evmAddress(RESEARCH_FEED_ADDRESS) }],
        },
      });
...
import { PublicClient, evmAddress, mainnet } from "@lens-protocol/client";
