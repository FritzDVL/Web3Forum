...
export function PublishStatusBadge({ status, lensPostId, contentUri, onRetry }: PublishStatusBadgeProps) {
  if (status === "confirmed" && lensPostId) {
    return (
      <span
        className="inline-flex items-center gap-1 rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700 dark:bg-green-900/30 dark:text-green-400"
        title={`Lens: ${lensPostId}`}
      >
        ✓ On-chain
      </span>
    );
